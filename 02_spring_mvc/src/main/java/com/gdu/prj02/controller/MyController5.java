...
public class MyController5 {
  ...
  // Spring 4 이후 GetMapping / PostMapping / PutMapping / DeletMappin 등 사용 가능
  ...
  public String add(RedirectAttributes redirectAttributes) {
    ...
    // add 결과를 falsh attribute 로 저장하면 redirect 경로에서 확인이 가능하다.
    ...
    // add 결과에 따른 이동
    String path = addResult == 1 ? "/faq/list.do" : "/main.do"; // 성공했을 때 /faq/list.do 로 실패했을땐 /main.do 주소로 
  ...
  @GetMapping("/faq/modify.do") // void 라 return 필요 없음
  public void modify(HttpServletRequest request, HttpServletResponse response) {
    
    // modify 결과
    int modifyResult = Math.random() < 0.5 ? 1 : 0; // 50퍼 확률로 성공 , 실패 설정
